...
void FIFSsort(int *a, int *b, int n);

void swap(int *, int*);

void SimulationFIFS(int *, int *, int *, int *, int *, int);
...
int main()
{
    ...
    FIFSsort(AT, BT, NumberOfProcesses);
    SimulationFIFS(AT, BT, CT, TAT, WT, NumberOfProcesses);
...
void SimulationFIFS(int *AT, int *BT, int *CT, int *TAT, int *WT, int n){
    if(n > 0){
        *CT = *AT + *BT;
        *TAT = *BT;
        *WT = 0;
